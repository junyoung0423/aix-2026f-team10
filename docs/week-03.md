...
| 상황 When/Where | 마음에 드는 옷을 발견했지만, 실제로 입었을 때 어떨지 그리고 이미 가지고 있는 옷들과 잘 어울릴지 알 수 없는 상황 | 수면 패턴이 무너지고 규칙적이지 못한다 | 재미로 혹은 관계 개선이 필요하다 혹은 그 사람을 더 이해하고싶다. |
| 페인포인트 What — 무엇이 **얼마나** | 실제 착용 모습과 기존 옷과의 색 조합을 확인할 수 없어 구매 후 안 입거나 반품하게 됨. | 피로한 상황이 이어지고아침에 일어나기 힘듦. | 그 사람에 대한 이해도 부족, 관계 개선 혹은 재미 |
| 성공 기준 Done | 피팅된 모델의 사진 또는 영상 | 개인에 맞춘 수면패턴 설정, 규칙적인 삶 확보 | 상대방의 성향 및 관계 향상을 위한 분석리포트 |

**한 문장으로 / One sentence each** — 네 칸을 이어 붙여 소리 내어 읽어 보세요.
형식: **[사용자]는 [상황]에서 [페인포인트] 때문에 [손해]를 본다.**
읽었을 때 어색하면 그 칸이 아직 덜 채워진 것입니다.

- A:옷을 소비하는 사람들은 온라인 쇼핑을 하는중에 실제로 입었을때 실패한다면 소비의 실패를 한다.
- B:잠을 잘 못자는 사람들은 수면 패턴을 지키지못했을 때 피로한 상황이 발생하여 더욱 피곤해진다.
...
| 질문 | 후보 A | 후보 B | 후보 C |
| --- | --- | --- | --- |
| 이 안에 **누구의 정보**가 들어가는가 그 사람이 우리 사용자와 같은 사람인가 | 옷을 소비하는 사람들의
키와 몸무게 | 수면 관리가 필요한 사람 | 관계 개선이 필요한 
사람들 |
| 쓰고 싶어도 **못 쓰는 사람**은 누구인가 | 없음 | 핸드폰이 없는 사람 ? | 그만큼 중요한 관계를 
이루지 못하고 있는 사람 |
